    ...
    if (state.motionTested == false) {
        return dynamicPage(name:"motionSetup", title:"Motion Detection Triggers", nextPage:"", refreshInterval:interval, install: true, uninstall: true){
            ...
            section("Diskstation Setup") {
                ...
                input "ignore2", "text", title:"", defaultValue:"Motion in %CAMERA%"
...
def getDSInfo() {
    ...
    // get cameras
    queueDiskstationCommand("SYNO.SurveillanceStation.Camera", "List", "", 9)
}
...
def makeCameraModelKey(cameraInfo) {
    def vendor = cameraInfo.vendor.replaceAll(" ", "%20")
    def model = cameraInfo.model.replaceAll(" ", "%20")
